...
export class UploadsService {
  ...
  private deleteLocal(files: any) {
    files.forEach((element: { filename: string }) => {
      fs.unlink(path.join('data/images', element.filename), (err) =>
        err
  ...
  async filesUpload(
    ...
    if (fileType === 'gameAvatar') {
      const dbFiles = await this.databaseService.getFilesByUserId(
        userId,
        fileType,
      );
      ...
      if (dbFiles) {
        ...
        dbFiles.forEach((f) =>
          takenPositions.push(JSON.parse(f.metadata).position),
        );
        takenPositions.sort((a, b) => a - b); // sort from min to max
        remainPositions = [1, 2, 3, 4].filter(
          (elem) => !takenPositions.includes(elem),
        ); // keep only in free fields
      }
    }
    ...
    for (const file of files) {
      ...
      if (fileType && fileType === 'gameAvatar') {
        context.metadata = JSON.stringify({
          position: remainPositions.shift(),
        }); // get only first and delete from arr
      }
