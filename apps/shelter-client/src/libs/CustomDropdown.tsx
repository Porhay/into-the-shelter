...
const CustomDropdown: React.FC<DropdownProps> = ({
  ...
  return (
    <div className={`login-container`} ref={dropdownRef}>
      {children}
      {isOpened && (
        <div className={`drop-down-content ${isTypeNotifications && 'notifications-content'}`}>
          {(type === 'account' || type === 'login') && <div className={'login-down-text'}>{text}</div>}
          {type === 'notifications' && <div className={'notifications-down-text'}>{text}</div>}
          {list.map((item, index) => (
